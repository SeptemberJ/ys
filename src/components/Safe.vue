  ...
  <div class="Safe">
    <el-form class="SafeForm MarginT_40" :model="formSafe" :rules="SafeRules" ref="formSafe" label-width="120px" label-position="right">
      <el-form-item
        ...
        label="手机号"
      >
        <el-input v-model="userFmobile" :disabled="true" placeholder="请输入注册的手机号" clearable></el-input>
      </el-form-item>
      <el-form-item
        class="TextAlignR"
        prop="code"
...
export default {
  ...
  computed: {
    ...mapState({
      userFmobile: state => state.userFmobile,
  ...
  methods: {
    ...
    getCode () {
      this.send({
        name: '/tokens/SMScode?fmobile=' + this.userFmobile,
        method: 'POST',
        data: {
        }
      }).then(res => {
      ...
      if (this.checkStatus === '2') {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.ifLoading = true
            this.send({
              name: '/tokens/registerChangePsw?fmobile=' + this.userFmobile + '&fpassword=' + this.formSafe.password,
              method: 'POST',
              data: {
              }
            }).then(res => {
